...
func AwaitSignals(l *net.TCPListener) os.Error {
	for {
		sig := <-signal.Incoming
...
func GetEnvs() (*net.TCPListener, int, os.Error) {
	envFd, err := os.Getenverror("GOAGAIN_FD")
	if nil != err { return nil, 0, err }
